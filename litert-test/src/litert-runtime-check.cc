...
std::vector<uint8_t> BuildAddModel() {
  ...
  const auto subgraph = tflite::CreateSubGraph(
      builder, builder.CreateVector(tensors),
  ...
  const auto model = tflite::CreateModel(
      builder, 3,
      builder.CreateVector<flatbuffers::Offset<tflite::OperatorCode>>(
          {op_code}),
      builder.CreateVector<flatbuffers::Offset<tflite::SubGraph>>({subgraph}),
      builder.CreateString("minimal add model for LiteRT runtime validation"),
      builder.CreateVector<flatbuffers::Offset<tflite::Buffer>>(
          {empty_buffer}));
...
int CheckRuntime(const char* model_path) {
  ...
  std::cout << "LiteRT runtime OK: " << lhs << " + " << rhs << " = "
            << output << "\n";
  std::cout << "TfLiteVersion via libLiteRt: " << TfLiteVersion() << "\n";
  return 0;
}
...
int main(int argc, char** argv) {
  const std::string model_path =
      argc > 1 ? argv[1] : "/tmp/litert-runtime-check/add.tflite";

  if (argc == 1) {
    if (mkdir("/tmp/litert-runtime-check", 0755) != 0 && errno != EEXIST) {
      std::cerr << "failed to create /tmp/litert-runtime-check\n";
      return 1;
    }
